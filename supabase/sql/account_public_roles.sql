-- Public account statuses for registration.
--
-- Visible statuses keep one of the existing permission roles:
--   user            -> regular profile owner
--   casting_agent   -> can create selections/book talent
--   admin           -> hidden platform staff roles
...
grant execute on function public.set_account_status(text)
  to authenticated;

create or replace function public.request_account_status(
  p_account_type text
)
returns void
language plpgsql
...
begin
  ...
  if public.role_for_account_type(v_account_type) = 'casting_agent' then
    insert into public.casting_agent_applications (
      user_id,
      status,
      requested_account_type,
...
create policy "Users can update own user role"
  ...
  for update
  to authenticated
  using (user_id::text = auth.uid()::text and lower(role) <> 'admin')
  with check (
    user_id::text = auth.uid()::text
    and lower(role) = 'user'
  );
